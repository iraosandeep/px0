...
1. Read-Only by Design: px0 is exclusively a code navigation and exploration tool by default. It does not write, edit, format, or mutate project files on disk in its default mode. The sole sanctioned exception is opt-in notes mode (`-notes`), which exposes `POST /api/save` — gated to same-origin/localhost requests (`localPost`), notes-mode-only, and Markdown-file-only (`isMarkdown` + `safePath`) — so the in-browser note editor can autosave. Do not introduce any other file modification or editor save API.
...
| Notes Mode (Markdown editing)        | `index.go`, `server.go`, `web/src/notesEditor.js`                | [`docs/internals/notes-mode.md`](../internals/notes-mode.md), [`README.md`](../../README.md)                    |
...
| `<div id="notesedit">`       | Notes mode (`-notes`) only: editable Markdown surface over `#viewport`, holding `<textarea id="notesedit-text">` and a small `#notesedit-status` save indicator. Autosaves on every keystroke via `POST /api/save`; never shown outside notes mode. |
...
| `web/src/notesEditor.js` | Notes mode (`-notes`) only: `openNote()`, `syncNotesEditor()`, `flushIfDirty()` — editable Markdown surface with debounced autosave to `POST /api/save`; excluded from `reloadOpenTabs()`. |
