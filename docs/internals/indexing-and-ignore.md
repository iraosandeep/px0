...
## 5. Notes Mode: `buildNotes()`

When `Index.notes` is true (set from the `-notes` CLI flag), `Build()` forks into `buildNotes()` instead of the walk described above. It restricts the tree to Markdown files and the directories that contain at least one anywhere in their subtree (Obsidian-style pruning), which is a bottom-up decision the top-down streaming walk above cannot make mid-traversal — `buildNotes()` is therefore a single-pass **recursive** walk that returns each directory's kept children before its parent decides whether to keep the directory. It also drops gitignored entries outright rather than listing them dimmed with `Ignored: true`, since their contents are never walked and a notes vault has no use for non-note clutter either way. See [notes-mode.md](notes-mode.md) for the full design and its interaction with `POST /api/save` and `reloadOpenTabs()`.
